...
/*
Find the total amount for each individual order that was spent on standard and gross papers in the orders table. 
This should give a dollar amount for each order in the order table.
*/

SELECT id, standard_amt_usd + gross_amt_usd AS standard_gross_total
FROM orders;
...
SELECT AVG(standard_qty) AS avg_standard_qty,
	   AVG(gross_qty) AS avg_gross_qty,
	   AVG(poster_qty) AS avg_poster_qty,
	   AVG(standard_amt_usd) AS avg_standard_amt,
	   AVG(gross_amt_usd) AS avg_gross_amt,
...
SELECT  a.name AS Acoount_name, SUM(o.total_amt_usd) AS Amount_spent
FROM orders o
INNER JOIN accounts a
...
SELECT CONVERT_TZ(NOW(), 'UTC','America/New_York') AS current_date_in_utc;
...
SELECT monthname(occurred_at), occurred_at
FROM orders;
...
SELECT month(occurred_at) AS month_no, monthname(occurred_at) AS month, occurred_at
FROM orders;
...
SELECT monthname(occurred_at) AS month_no, SUM(total_amt_usd) AS orders_amount
FROM orders
GROUP BY monthname(occurred_at)
ORDER BY orders_amount DESC;
