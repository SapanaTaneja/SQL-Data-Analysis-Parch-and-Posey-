...
FROM
	(SELECT channel, FORMAT(occurred_at, 'MM-DD-YY') AS day, COUNT(*) AS num_events
	FROM web_events
	GROUP BY  channel, FORMAT(occurred_at, 'MM-DD-YY')) AS events_table
GROUP BY channel
...
FROM orders
WHERE concat(month(occurred_at), '-', year(occurred_at))=
	(SELECT min(concat(month(occurred_at), '-', year(occurred_at))) as Month_Year
	FROM orders);
...
FROM
	(SELECT  a.name AS account_name, SUM(o.total_amt_usd) AS total_spent
	FROM orders o
	INNER JOIN accounts a
	ON a.id = o.account_id
	GROUP BY a.name
	ORDER BY total_spent DESC
    limit 10) top_ten_customer;
...
WITH all_events AS (
	SELECT channel, Date(occurred_at) AS day, COUNT(*) AS num_events
	FROM web_events
	GROUP BY  channel, Date(occurred_at))
SELECT channel, AVG(num_events) AS avg_events
FROM all_events
GROUP BY channel
ORDER BY avg_events DESC;
